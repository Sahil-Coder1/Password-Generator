...
if(isNumber && isSymbol){
  ...
  <div className=' mx-20 text-center content-center p-2 rounded-xl bg-slate-400'>
  <input type="text" name="n" id="pass" value={pass} className='bg-green-200 p-3 rounded-lg text-4xl text-center w-fit' disabled/>
  <button className='bg-green-600 text-4xl text-white p-3 rounded-xl ml-2 hover:bg-green-500 ' onClick={() => {Copy()}}>Copy</button>
  <div>
    <div className='  flex-2 content-center'>
    <label  className='text-orange-800 text-2xl'>Length : {len}</label> &nbsp;
  <input type="range" id="vol" name="vol" min="10" max="20" value={len} onChange={(e) => {Password(); setLength(e.target.value) } }/> &nbsp;
  ...
  <button className='bg-yellow-600 text-2xl text-white p-4 rounded-xl m-2 hover:bg-yellow-500' onClick={() => {Password()}}>Generate</button>
  </div>
  </div>
   </div>
    </div>
